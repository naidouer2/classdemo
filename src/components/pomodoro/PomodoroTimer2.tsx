...
export default function PomodoroTimer2() {
  ...
  }, [isExpanded, toggleTimer, resetTimer])
    ...
    <>
      ...
      {!isExpanded && (
        <button
          ...
          className="fixed bottom-6 right-6 w-16 h-16 rounded-full bg-gradient-to-br from-pink-500 via-purple-500 to-indigo-500 shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-110 flex items-center justify-center group animate-bounce"
          style={{
            boxShadow: '0 0 20px rgba(236, 72, 153, 0.4), 0 0 40px rgba(139, 92, 246, 0.3), 0 0 60px rgba(99, 102, 241, 0.2)'
          }}
        >
          <div className="w-14 h-14 rounded-full bg-white/10 backdrop-blur-sm flex items-center justify-center group-hover:scale-105 transition-transform"
          >
          ...
          {state.sessionCount > 0 && (
            <div className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-gradient-to-r from-pink-500 to-red-500 flex items-center justify-center text-white text-xs font-bold animate-pulse"
            >
              {state.sessionCount}
            </div>
          )}
      ...
      {isExpanded && (
        ...
        >
          <div 
            className="relative bg-gradient-to-br from-purple-400/30 via-pink-400/30 to-orange-400/30 backdrop-blur-xl rounded-3xl p-6 shadow-2xl border border-white/20 transform transition-all duration-500 hover:scale-105"
            style={{
              boxShadow: 
                '0 0 30px rgba(236, 72, 153, 0.4), 0 0 60px rgba(139, 92, 246, 0.3), 0 0 90px rgba(245, 158, 11, 0.2)',
            }}
          >
            {/* 关闭按钮 */}
            <button
              onClick={() => setIsExpanded(false)}
              className="absolute -top-2 -right-2 w-8 h-8 rounded-full bg-gradient-to-r from-pink-500 to-purple-500 text-white flex items-center justify-center hover:scale-110 transition-transform shadow-lg"
            >
            ...
            >
              💡 快捷键: <kbd className="px-2 py-1 bg-white/20 rounded mx-1">空格</kbd> 暂停/开始 | 
              <kbd className="px-2 py-1 bg-white/20 rounded mx-1">Esc</kbd> 关闭面板
            </div>
          </div>
        </div>
      )}
